		...
		if move is not None and move != "":
			...
			try:
				...
				if not s.board.is_game_over():
					...
					'''
					action = player.action(s, num_halfmoves, graph = graph)
					print("Computer moves: ", action)
					#labels = util.labels
					...
					#print("value: ", value[0], "Policy: ", action)
					#s.board.push(chess.Move.from_uci(labels[np.argmax(policy)]))
					#s.board.push(chess.Move.from_uci(labels[action]))
					s.board.push_uci(action)
			except Exception:
				traceback.print_exc()
